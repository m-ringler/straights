...
public class GridFinderTests
{
    ...
    public void FindGrid(string fileName, bool skewed = false)
    {
        ...
        // ACT
        var grid = new GridFinder(this.DebugInfoWriter).FindGrid(png);
        ...
        _ = grid
            .VerticalLines.Should()
            .Equal(grid.VerticalLines.OrderBy(x => x.Rho));
        _ = grid
            .HorizontalLines.Should()
            .Equal(grid.HorizontalLines.OrderBy(x => x.Rho));
        ...
        if (!skewed)
        {
            ...
            for (int i = 0; i <= 9; i++)
            {
                _ = v[i]
                    .Rho.Should()
                    .BeApproximately(v[0].Rho + (i * cw), 6.0f);
                _ = h[i]
                    .Rho.Should()
                    .BeApproximately(h[0].Rho + (i * ch), 6.0f);
