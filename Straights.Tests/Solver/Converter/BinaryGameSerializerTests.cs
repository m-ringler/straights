...
public class BinaryGameSerializerTests
{
    ...
    public void ToBinary4x4()
    {
        ...
        // ACT
        var actual = GridConverter.ToBinaryString(
            solved,
            unsolved,
    ...
    private static void ShouldEqual(
        Grid<SolverField> expected,
        Grid<SolverField> actual
    )
    {
    ...
    {
        var rng = new RandNRandomFactory().CreatePcg32(
            "Pcg32-aa6eb674cf4465ab-ad4c4f2aa9acaba8"
        );
        ...
        var generator = new DifficultyAdjuster(solver.GridSimplifier.ToSolver())
        {
            ...
            new GridGenerator(
                solver,
                new RandomEmptyGridGenerator(parameters)
                {
                    RandomNumberGenerator = rng,
                }
            )
