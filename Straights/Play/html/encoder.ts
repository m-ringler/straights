...
export class BitmaskEncoder {
    ...
    async encode(
        ...
        for (const set of setsArray) {
            ...
            for (const value of set) {
                if (value < 1 || value > n) {
                    throw new Error(
                        `Set contains invalid value ${value}: must be between 1 and ${n}`
                    );
                }
            }
        ...
        if (totalBytes - 1 > this.compressionThreshold) {
            ...
            const compressionRatio =
                (compressed.length - 1) / dataToCompress.length; // Subtract control byte from comparison
    ...
    async decode(encoded: EncodedResult, n: number): Promise<Set<number>[]> {
        ...
        if (
            controlByte !== CONTROL_BYTE_UNCOMPRESSED &&
...
function fromBase64Url(base64url: string): Uint8Array {
    ...
    if (typeof (Uint8Array as any).fromBase64 === 'function') {
        return (Uint8Array as any).fromBase64(base64url, {
            alphabet: 'base64url',
        });
    }
