...
// Button Functions
    ...
    _positionPopup(target: Element | JQuery<HTMLElement> | ArrayLike<Element>, popup: JQuery<HTMLElement>) {
    ...
    _renderUndoButton(length: number) {
...
// General Functions
    _changeGridSize(newGridSize: number): void {
    ...
    _getURLParameter(name: string) {
        ...
        _removeURLParameter(paramKey: string): void {
    ...
    try {
            this._generateGridSize = loadSetting('grid-size-slider', 'generate.gridSize', DEFAULT_GRID_SIZE)
            this._difficulty = loadSetting('difficulty-slider', 'generate.difficulty', DEFAULT_DIFFICULTY)
    } catch (error) {
    ...
    _handleCursorKey(e: { which: number }) {
    switch (e.which) {
        case 37: // left
    ...
    _handleGameLoad(popstate = false) {
    ...
    _onResize() {
        ...
        if (window.innerWidth / 2 - 45 < $('.controls').position().left) { // Large screen
            ...
            const cellwidth = Math.min(Math.floor(window.innerWidth / this._currentGridSize - 2), 41)
            $('#buttons-small').show()
            $('#buttons-large').hide()
...
$(async function () {
    ...
    $('td[id^="ce"]').on('click', function () { // Game fields
        ...
        _ui.selectCell(row, col)
    })
    $('td[data-button^="bn"]').on('click', function () { // Number buttons
        const num = Number($(this).text())
        _ui._handleNumberInput(num)
    })
    _ui._onResize()
})

window.addEventListener('popstate', function () {
    _ui._handleGameLoad(true)
})

$(document).on('keydown', function (e) { _ui._onKeyDown(e as any) });

$(window).on('resize', function () { _ui._onResize() });
